import { div } from 'framer-motion/client'; // Dit lijkt niet correct. Misschien is dit een fout, aangezien 'div' geen geëxporteerde component is in framer-motion.
import React from 'react';
import QuestionBubble from '../assets/QuestionBubble.png'; // Afbeelding van het vraagballonnetje
import learn from '../assets/learn.png'; // Deze afbeelding wordt niet gebruikt, mogelijk overbodig
import { useRef, useState } from "react"; // React hooks voor het beheren van referenties en staat
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'; // Voor FontAwesome-iconen
import { faMagnifyingGlass, faArrowDown, faWrench } from "@fortawesome/free-solid-svg-icons"; // De iconen die worden gebruikt op de pagina

function StartLearningPage() {

    // Ref voor de FAQ-sectie, zodat we de hoogte ervan kunnen aanpassen
    const faqRef = useRef(null);
    const [isExpanded, setIsExpanded] = useState(false); // Bijhouden of de FAQ-sectie is uitgeklapt of niet
    ...
    // Functie om de FAQ-sectie in- of uit te klappen
    const toggleFAQ = () => {
        const content = faqRef.current;
        if (!content) return;

        // Als de FAQ niet is uitgeklapt, stel de maximale hoogte in op de scrollhoogte (om het volledig weer te geven)
        if (!isExpanded) {
            content.style.maxHeight = content.scrollHeight + "px";
        } else {
            // Als de FAQ is uitgeklapt, stel de hoogte in zodat het er mooi uitziet bij in- en uitklappen
            content.style.maxHeight = content.scrollHeight + "px";
            setTimeout(() => {
                content.style.maxHeight = "200px"; // Standaard hoogte voor ingeklapt gedeelte
            }, 10);
        }

        setIsExpanded(!isExpanded); // De status van uitklappen wordt omgekeerd
    };

    return (
        <div className="pt-[5vh] w-full h-auto flex flex-col items-center">
            {/* Hoofdsectie met zoekbalk en afbeelding */}
                ...
                <div className="w-full flex flex-col md:flex-row gap-[20px] justify-around items-center p-[30px] min-h-[80vh] max-h-fit">
                    <div className="w-full lg:mt-[50px] lg:w-3/5 h-auto flex flex-col gap-[20px] md:w-[65vw] md:mr-[50px]">
                        <div className="w-full md:w-[70%] flex flex-col items-start justify-center mt-[20px]">
                            <h1 className="font-bold text-[32px] md:text-[40px] text-6xl mb-[20px]">We are here for all your questions</h1> {/* Titel */}
                        </div>
                        <p className="w-full md:w-[85%] text-[16px] md:text-[24px] mb-[20px] text-xl">
                            Lorem ipsum dolor, sit amet consectetur adipisicing elit. {/* Korte uitleg over de pagina */}
                        </p>
                        <p className="w-full md:w-[85%] text-[12px] md:text-[18px] text-xl">
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. {/* Verdere uitleg */}
                        </p>
                        {/* Zoekbalk */}
                        <input 
                    ...
                    {/* Afbeelding aan de rechterkant van het scherm */}
            ...
            <div className="w-full bg-[#1BB1AB] mt-[30px] flex flex-col items-center pb-10 min-h-[71vh] p-[30px]">
                ...
                {/* FAQ-vragen, in- of uitgeklapt */}
                <div 
                    ref={faqRef}
                    className="w-[70%] overflow-hidden transition-all duration-500 ease-in-out"
                    style={{ maxHeight: "200px" }} // Standaard ingeklapte hoogte
                >
                    {faqQuestions.map((question, index) => (
                        <div key={index} className="flex flex-row w-full items-center gap-[20px] mt-[30px]">
                            <FontAwesomeIcon icon={faMagnifyingGlass} className="text-white text-xl md:text-2xl" /> {/* Vergrootglas-icoon */}
                            <p className="text-white text-[18px]">{question}</p> {/* Vraagtekst */}
                        </div>
                    ))}
                </div>
                {/* Toggle knop om meer of minder FAQ's weer te geven */}
            ...
            {/* Sectie over waarom je voor Aithentic zou moeten kiezen */}
