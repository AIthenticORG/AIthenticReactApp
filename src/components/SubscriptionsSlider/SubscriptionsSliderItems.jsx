...
const SubscriptionsSliderItems = ({ from, to }) => {
  // Slider items met abonnementsgegevens
  const sliderItems = [
    { title: "Basis",
  ...
  return (
    // Container voor de motion divs
    <div className="flex overflow-x-hidden w-full h-auto">
      {/* Herhaal de motion.div voor de animatie */}
      {[...Array(3)].map((_, i) => (
        <motion.div
          key={i}
          initial={{ x: from }} // Startpositie van de slider
          animate={{ x: to }}   // Eindpositie van de slider
          ...
          {/* Render de items binnen de motion.div */}
