...
const AssociatedCompanies = () => {
    // Definieer de beschikbare types voor de slider (one, two, three)
    const types = ['one', 'two', 'three'];
    
    // Staat voor het bijhouden van de huidige index van het geselecteerde type
    const [currentIndex, setCurrentIndex] = useState(0);
    
    // Staat voor het bijhouden van het huidige type
    const [currentType, setCurrentType] = useState(types[0]);

    // Functie om naar het volgende bedrijf in de slider te gaan
    const handleNext = () => {
        setCurrentIndex((prevIndex) => {
            const newIndex = (prevIndex + 1) % types.length;  // Circulair index verhogen
            setCurrentType(types[newIndex]);
            return newIndex;
        });
    };

    // Functie om naar het vorige bedrijf in de slider te gaan
    const handlePrevious = () => {
        setCurrentIndex((prevIndex) => {
            const newIndex = (prevIndex - 1 + types.length) % types.length;  // Circulair index verlagen
            setCurrentType(types[newIndex]);
            return newIndex;
        });
    };
    ...
    return (
        <div className='w-full h-full p-6 flex items-center justify-center'>
            {/* Voorwaardelijke rendering op basis van currentType */}
            {currentType === 'one' && (
                <div className='bg-white p-8 rounded-lg  flex flex-col items-center max-w-[900px] h-auto'>
                    <h2 className='text-gray-600 text-lg'>Geassocieerde bedrijven</h2>
                    <h1 className='text-2xl font-bold mt-2'>Onze huidige selectie</h1>
                    <div className='flex flex-col sm:flex-row mt-6 w-full'>
                        {/* Bedrijfsafbeelding aan de linkerkant */}
                        ...
                        {/* Beschrijving en details aan de rechterkant */}
